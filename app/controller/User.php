...
class User extends Common
{
    ...
    public function userList()
    {
        if ($this->request->isAjax()) {
            ...
            $list = UserModel::withSearch(['name'], ['name' => $data['key']])
                ->hidden(['password'])
                ->paginate(['query' => $data,'list_rows' => $data['limit']], false, );
            $user_date = [];
    ...
    {
        if (!$this->request->isAjax()) {
            $info = UserModel::find($this->uid)->hidden(['password']);
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            $grouplist = AuthGroup::column('title','id');
    ...
    public function groupList()
    {
        if ($this->request->isAjax()) {
            ...
            $list = AuthGroup::where($map)->paginate(['query' => ['key' => $key], 'list_rows' => $limit], false);
    ...
    public function editGroup()
    {
        if ($this->request->isPost()) {
            ...
            if ($id) {//编辑用户组
                $res = AuthGroupService::edit($id, ['title' => $title]);
                return $res;
            } else {//添加用户组
