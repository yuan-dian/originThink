...
class System extends Common
{
    ...
    public function cleanCache()
    {
        ...
        if (!$this->request->isPost()) {
            ...
            if (isset($data['path'])) {
                ...
                return show([], 1, '缓存清空成功');
            } else {
                return show([], 0, '请选择清除的范围');
            }
        }
    }
    ...
    public function editMenu()
    {
        if ($this->request->isPost()) {
            ...
            if ($res) {
                $cache_tag = config('auth.cache_tag');
                Cache::tag($cache_tag)->clear();//清除Auth类设置的缓存
                return show([], 1, '保存成功');
            } else {
                return show([], 0, '保存失败');
            }
            ...
            $menu = AuthRule::where('pid', '=', 0)->order('sort desc')->column('title', 'id');
            $menu[0] = '顶级菜单';
            ksort($menu);
