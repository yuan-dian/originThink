...
class AuthService
{
    ...
    public function getGroups()
    {
        ...
        // 执行查询
        $user_groups = Db::name($this->config['auth_group'])->where('id', 'in', $this->group_id)->where('status','=',1)->select();
        if ($is_cache) {
            //设置缓存
    ...
    public function getMenuList()
    {
        ...
        if (!$super_admin) { //不是超级管理员，根据用户组获取对应的菜单
            ...
            $menuList = Db::name($this->config['auth_rule'])->where($map)->order('sort desc')->select();

            $menuList = list_to_tree($menuList->toArray());
        }
        if ($is_cache) {
            //设置缓存数据
